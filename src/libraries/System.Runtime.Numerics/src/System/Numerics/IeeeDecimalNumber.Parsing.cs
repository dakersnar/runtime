...
namespace System.Numerics
{
    internal static partial class IeeeDecimalNumber
    {
        ...
        {
            if (!TryParseDecimal64(value, styles, info, out Decimal64 result))
            {
        ...
        {
            IeeeDecimalNumberBuffer number = new IeeeDecimalNumberBuffer(stackalloc byte[Decimal64BufferLength]);
            ...
            {
                result = NumberToDecimal64(ref number);
            }
...
/*                        if (number.Kind != NumberBufferKind.Decimal)
        ...
        internal static unsafe Decimal64 NumberToDecimal64(ref IeeeDecimalNumberBuffer number)
        {
            ...
            if ((number.DigitsCount == 0) || (number.Scale < Decimal64MinExponent)) // TODO double check this
            {
                // TODO are we sure this is the "right" zero to return in all these cases
                return number.IsNegative ? Decimal64.NegativeZero : Decimal64.Zero;
            }
            else if (number.Scale > Decimal64MaxExponent) // TODO double check this
            {
                return number.IsNegative ? Decimal64.NegativeInfinity : Decimal64.PositiveInfinity;
            }
            else
            {
                ...
                // value = c * 10 ^ q
                // Which means
                // c = first N digits of Mantissa, where N is min(Decimal64.Precision, number.DigitsCount)
                // q = Exponent - N
